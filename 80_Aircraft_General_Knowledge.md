# 80 Aircraft General Knowledge

The correct option is marked like this:

- [x] correct option


**1.** The load factor "n" describes the relationship between...

- [ ] drag and lift.
- [ ] weight and thrust.
- [x] lift and weight.
- [ ] thrust and drag.

**2.** The fuselage structure may be damaged by...

- [ ] airspeed decreasing below a certain value.
- [x] exceeding the manoeuvering speed in heavy gusts.
- [ ] neutralizing stick forces according to actual flight state.
- [ ] stall after exceeding the maximum angle of attack.

**3.** The thickness of the wing is defined as the distance between the lower and the upper side of the wing at the...

- [ ] thinnest part of the wing.
- [x] thickest part of the wing.
- [ ] most outer part of the wing.
- [ ] most inner part of the wing.

**4.** The sandwich structure consists of two...

- [ ] thick layers and a heavy core material.
- [x] thin layers and a light core material.
- [ ] thin layers and a heavy core material.
- [ ] thick layers and a light core material.

**5.** Which are the advantages of sandwich structures?

- [ ] Good formability and high temperature durability
- [ ] High temperature durability and low weight
- [x] Low weight, high stiffness, high stability, and high strength
- [ ] High strength and good formability

**6.** What is a tubular steel construction with a non self-supporting skin referred to as?

- [x] Grid construction
- [ ] Monocoque construction
- [ ] Semi-monocoque construction.
- [ ] Honeycomb structure

**7.** What are the major components of an aircraft's tail?

- [x] Horizontal tail and vertical tail
- [ ] Rudder and ailerons
- [ ] Ailerons and elevator
- [ ] Steering wheel and pedals

**8.** Which constructional elements give the wing its profile shape?

- [ ] Tip
- [x] Ribs
- [ ] Spar
- [ ] Planking

**9.** Which of the stated materials shows the highest strength?

- [ ] Wood
- [x] Carbon fiber re-inforced plastic
- [ ] Magnesium
- [ ] Aluminium

**10.** A construction made of frames and stringer with a supporting skin is called...

- [ ] Wood- or mixed construction.
- [ ] Grid construction.
- [ ] Honeycomb structure.
- [x] Semi-monocoque construction.

**11.** Primary fuselage structures of wood or metal planes are usually made up by what components?

- [ ] Covers, stringers and forming parts
- [ ] Girders, ribs and stringers
- [x] Frames and stringer
- [ ] Ribs, frames and covers

**12.** Reasons for dents in the aircraft structure are...

- [x] hard landing or excessive stress.
- [ ] material defects or old colour.
- [ ] excessive engine rpm and cylinder defects.
- [ ] intense erosion or high wear.

**13.** Number 1 on the gear designates the...
  
![AGK-005](https://raw.githubusercontent.com/antiprosynthesis/ppl-a/main/figures/AGK-005.jpg)

- [ ] inner shock absorber.
- [ ] torque link.
- [x] fork.
- [ ] strut.

**14.** How is the nose or tail wheel usually controlled on small aircraft and motor gliders?

- [ ] By weight movement
- [ ] By the control column
- [x] By the pedals
- [ ] By the steering wheel

**15.** What kind of control surface is connected with the nose wheel?

- [ ] Trim rudder
- [ ] Elevator
- [ ] Aileron
- [x] Rudder

**16.** What are the primary and the secondary effects of a rudder input to the left?

- [x] Primary: yaw to the left Secondary: roll to the left
- [ ] Primary: yaw to the right Secondary: roll to the left
- [ ] Primary: yaw to the left Secondary: roll to the right
- [ ] Primary: yaw to the right Secondary: roll to the right

**17.** About how many axes does an aircraft move and what are these axes called?

- [ ] 4; optical axis, imaginary axis, sagged axis, axis of evil
- [ ] 3; x-axis, y-axis, z-axis
- [x] 3; vertical axis, lateral axis, longitudinal axis
- [ ] 4; vertical axis, lateral axis, longitudinal axis, axis of speed

**18.** What is the effect of pulling the control yoke or stick backwards?

- [ ] The aircraft's tail will produce an increased downward force, causing the aircraft's nose to drop
- [x] The aircraft's tail will produce an increased downward force, causing the aircraft's nose to rise
- [ ] The aircraft's tail will produce an decreased upward force, causing the aircraft's nose to drop
- [ ] The aircraft's tail will produce an increased upward force, causing the aircraft's nose to rise

**19.** A movement around the longitudinal axis is primarily initiated by the...

- [x] ailerons.
- [ ] elevator.
- [ ] trim tab.
- [ ] rudder.

**20.** What is the purpose of the secondary flight controls?

- [ ] To constitute a backup system for the primary flight controls
- [x] To improve the performance characteristics of an aircraft and relieve the pilot of excessive control forces
- [ ] To improve the turn characteristics of an aircraft in the low speed regime during approach and landing
- [ ] To enable the pilot to control the aircraft's movements about its three axes

**21.** When trimming an aircraft nose up, in which direction does the trim tab move?

- [ ] In direction of rudder deflection
- [ ] Depends on CG position
- [x] It moves down
- [ ] It moves up

**22.** The trim wheel or lever in the cockpit is moved aft by the pilot. What effect does this action have on the trim tab and on the elevator?

- [x] The trim tab moves down, the elevator moves up
- [ ] The trim tab moves up, the elevator moves up
- [ ] The trim tab moves up, the elevator moves down
- [ ] The trim tab moves down, the elevator moves down

**23.** What is the purpose of the tank ventilation?

- [x] To prevent underpressure caused by fuel consumption
- [ ] To prevent water disposal during parking
- [ ] To distribute the fuel from one tank segment to the other during flight
- [ ] To prevent fuel spillage during refueling on the filler plug

**24.** Where does the condensation water converge in the tank?

- [ ] Near the cap of the tank
- [ ] It floats on the fuel
- [x] At the lowest position
- [ ] It is mixed with the fuel

**25.** What is the unit for electrical power?

- [ ] Ohm
- [ ] Ampere
- [ ] Volt
- [x] Watt

**26.** What must be considered during refueling?

- [ ] Check the fuel content of the tank with a torch and remove fire protection
- [x] No open fires, obey smoking ban and apply ground cables
- [ ] Apply ground wires, turn on the main switch and magneto ignition
- [ ] Refuel through a soaked rag and keep a fire extinguisher available

**27.** What is the reason for static dischargers on aircraft?

- [ ] To eliminate electrical interferences during intensive radio traffic
- [x] To discharge static charging during flight
- [ ] To improve the quality of radio transmission in high altitudes
- [ ] To ensure grounding during refueling

**28.** An aeroplane's current supply is carried out by the: 1. Battery 2. Generator 3. Relay 4. Circuit breaker

- [x] 1 and 2
- [ ] 1 and 4
- [ ] 2 and 3
- [ ] 3 and 4

**29.** What devices can be affected by a failure of the electrical system?

- [ ] Fuel quantity indication, radio equipment and altimeter
- [ ] Radio equipment, navigation equipment and magnetic compass
- [ ] Airspeed indicator, altimeter and artificial horizon
- [x] Radio equipment, navigation equipment and gyros

**30.** What engines are commonly used with Touring Motor Gliders (TMG)?

- [ ] 2 plate Wankel
- [ ] 2 Cylinder Diesel
- [ ] 4 Cylinder 2 stroke
- [x] 4 Cylinder; 4 stroke

**31.** Which part of the cycle of a four-stroke Otto engine can be seen in the attachment?
  
![AGK-008](https://raw.githubusercontent.com/antiprosynthesis/ppl-a/main/figures/AGK-008.jpg)

- [ ] Fourth stroke - exhaust
- [ ] First stroke - induction
- [ ] Third stroke - power
- [x] Second stroke - compression

**32.** Which part of the cycle of a four-stroke Otto engine can be seen in the attachment?
  
![AGK-007](https://raw.githubusercontent.com/antiprosynthesis/ppl-a/main/figures/AGK-007.jpg)

- [ ] Fourth stroke - exhaust
- [ ] Third stroke - power
- [ ] Second stroke - compression
- [x] First stroke - induction

**33.** Which colour does Avgas 100 LL have?

- [ ] Green
- [ ] Yellow
- [x] Blue
- [ ] Red

**34.** What is the secondary effect of switching on the carburettor heating on fixed propeller engines during engine run-up?

- [x] RPM decreases
- [ ] RPM increaes
- [ ] Angle of attack increases
- [ ] Angle of attack decreases

**35.** In which phase of flight must the carburettor heating be switched off although carburettor icing might be expected?

- [ ] During climb
- [ ] During taxi
- [x] During take-off
- [ ] During cruise

**36.** What is the task of cooling fins on air-cooled engine cylinders?

- [x] Quick heat transfer to the surrounding air flow by expanding the surface
- [ ] Leading the airflow to parts designated to be cooled
- [ ] Cooling of the cylinder surrounding airflow and forwarding to hotter engine parts
- [ ] Increasing the airflow thus improving cooling of cylinder parts

**37.** What happens during oil filter clogging?

- [ ] The oil circulation will end after 15 minutes so that a proper engine run will not be guaranteed
- [ ] The oil circulation will end after 30 minutes so that a proper engine run will not be guaranteed
- [ ] A bypass valve opens thus enabling the circulation to continue, debris will be filtered by an alternate filter
- [x] A bypass valve opens thus enabling the circulation to continue, debris will not be filtered

**38.** What is the purpose of engine oil in piston engine?

- [ ] For nose reduction and bearing of the propeller shaft.
- [x] Cooling, lubrication of the engine and disposal of fine abrasion.
- [ ] Cooling of the engine and lubrication of propeller shaft.
- [ ] Cooling, lubrication and increasing engine performance.

**39.** What is the likely cause if the engine runs unusually rough while checking the magnetos?

- [ ] Ignition switch is faulty
- [ ] Short circuit on the ground cable
- [ ] Starter is faulty
- [x] Sparking plug is defective

**40.** How do you call fuel mixtures with a high amount of fuel?

- [ ] Empty
- [x] Rich
- [ ] Lean
- [ ] Full

**41.** With increasing altitude and unchanged mixture setting, the air/fuel mixture...

- [ ] becomes liquid.
- [ ] stays constant.
- [x] becomes richer.
- [ ] becomes more lean.

**42.** How should an Otto engine be shut down?

- [ ] By moving the propeller to the feathered position
- [x] By completely leaning the mixture
- [ ] By closing the throttle lever completely
- [ ] By shutting down the generator

**43.** Which gauges involve a measurement of temperature?

- [ ] Engine lubricant, exhaust gas, outside air, cabin air, directional gyro
- [ ] Engine lubricant, cylinder head, exhaust gas, suction, outside air, cabin air
- [x] Engine lubricant, engine cooling fluid, cylinder head, exhaust gas, outside air, cabin air
- [ ] Engine oil, cylinder head, exhaust gas, altimeter, airspeed indicator, vertical speed indicator

**44.** Fuel quantity in small aircraft is most commonly measured through a level sensor. The European Certification Specifications CS 23 require that...

- [x] a fuel quantity indicator must be available for each tank and the indicator must be calibrated to read "zero" during level flight when the quantity of fuel remaining in the tank is equal to the unusable fuel supply.
- [ ] if no fuel quantity indicator is available to the pilot in flight, the pilot must check the fuel quantity before commencing the flight and recalculate the remaining fuel on board in regular intervals during the flight.
- [ ] if the fuel indicator fails, the pilot must land at the next available airport.
- [ ] at least one fuel quantity indicator must be available to indicate the total amount of fuel abord an aircraft. This Indicator must be calibrated adequately to indicate the correct amount of fuel during all phases of flight.

**45.** What does the dynamic pressure depend directly on?

- [ ] Air density and lift coefficient
- [ ] Lift- and drag coefficient
- [x] Air density and airflow speed squared
- [ ] Air pressure and air temperature

**46.** The term "static pressure" is defined as pressure...

- [ ] sensed by the pitot tube.
- [x] of undisturbed airflow.
- [ ] resulting from orderly flow of air particles.
- [ ] inside the airplane cabin.

**47.** Which cockpit instruments are connected to the static port?

- [ ] Airspeed indicator, altimeter, direct-reading compass
- [x] Altimeter, vertical speed indicator, airspeed indicator
- [ ] Airspeed indicator, direct-reading compass, slip indicator
- [ ] Altimeter, slip indicator, navigational computer

**48.** The Pitot / static system is required to...

- [x] measure total and static air pressure.
- [ ] correct the reading of the airspeed indicator to zero when the aircraft is static on the ground.
- [ ] prevent icing of the Pitot tube.
- [ ] prevent potential static buildup on the aircraft.

**49.** Which pressure is sensed by the Pitot tube?

- [ ] Static air pressure
- [x] Total air pressure
- [ ] Cabin air pressure
- [ ] Dynamic air pressure

**50.** Which of the mentioned cockpit instruments is connected to the pitot tube?

- [ ] Altimeter
- [x] Airspeed indicator
- [ ] Vertical speed indicator
- [ ] Direct-reading compass

**51.** The altimeter's reference scale is set to airfield pressure (QFE). What indication is shown during the flight?

- [ ] Altitude above MSL
- [ ] Airfield elevation
- [x] Height above airfield
- [ ] Pressure altitude

**52.** A true altitude is...

- [ ] a pressure altitude corrected for non-standard temperature.
- [ ] a height above ground level corrected for non-standard pressure.
- [x] an altitude above mean sea level corrected for non-standard temperature.
- [ ] a height above ground level corrected for non-standard temperature.

**53.** Which instrument can be affected by the hysteresis error?

- [ ] Vertical speed indicator
- [x] Altimeter
- [ ] Direct reading compass
- [ ] Tachometer

**54.** Which is the purpose of the altimeter subscale?

- [ ] To set the reference level for the altitude decoder of the transponder
- [x] To reference the altimeter reading to a predetermined level such as mean sea level, aerodrome level or pressure level 1013.25 hPa
- [ ] To correct the altimeter reading for system errors
- [ ] To adjust the altimeter reading for non-standard temperature

**55.** The measurement of altitude is based on the change of the...

- [x] static pressure.
- [ ] total pressure.
- [ ] dynamic pressure.
- [ ] differential pressure.

**56.** In which way may an altimeter subscale which is set to an incorrect QNH lead to an incorrect altimeter reading?

- [ ] If the subscale is set to a lower than actual pressure, the indication is too high. This may lead to much closer proximity to the ground than intended
- [ ] If the subscale is set to a lower than actual pressure, the indication is too low. This may lead to much closer proximity to the ground than intended
- [x] If the subscale is set to a higher than actual pressure, the indication is too high. This may lead to much closer proximity to the ground than intended
- [ ] If the subscale is set to a higher than actual pressure, the indication is too low. This may lead to much greater heights above the ground than intended

**57.** When is it necessary to adjust the pressure in the reference scale of an alitimeter?

- [ ] After maintance has been finished
- [x] Before every flight and during cross country flights
- [ ] Once a month before flight operation
- [ ] Every day before the first flight

**58.** Lower-than-standard temperature may lead to...

- [ ] a blockage of the Pitot tube by ice, freezing the altimeter indication to its present value.
- [ ] a correct altitude indication as long as the altimeter subscale is set to correct for non-standard temperature.
- [x] an altitude indication which is too high.
- [ ] an altitude indication which is too low.

**59.** What difference in altitude is shown by an altimeter, if the reference pressure scale setting is changed from 1000 hPa to 1010 hPa?

- [ ] 80 m less than before
- [ ] Zero
- [ ] Values depending on QNH
- [x] 80 m more than before

**60.** Which of the following options states the working principle of a vertical speed indicator?

- [x] Measuring the present static air pressure and comparing it to the static air pressure inside a reservoir
- [ ] Static air pressure is measured and compared against a vacuum
- [ ] Total air pressure is measured and compared to static pressure
- [ ] Measuring the vertical acceleration through the displacement of a gimbal-mounted mass

**61.** A vertical speed indicator measures the difference between...

- [x] instantaneous static pressure and previous static pressure.
- [ ] dynamic pressure and total pressure.
- [ ] total pressure and static pressure.
- [ ] instantaneous total pressure and previous total pressure.

**62.** A vertical speed indicator connected to a too big equalizing tank results in...

- [ ] indication too low
- [ ] mechanical overload
- [x] indication too high
- [ ] no indication

**63.** The vertical speed indicator measures the difference of pressure between...

- [ ] the present dynamic pressure and the dynamic pressure of a previous moment.
- [x] the present static pressure and the static pressure of a previous moment.
- [ ] the present dynamic pressure and the static pressure of a previous moment.
- [ ] the present total pressure and the total pressure of a previous moment.

**64.** QNE is the...

- [ ] magnetic bearing to a station.
- [x] altitude above the reference pressure level 1013.25 hPa.
- [ ] barometric pressure at a reference datum, typically the runway threshold of an airfield.
- [ ] barometric pressure adjusted to sea level, using the international standard atmosphere (ISA).

**65.** QFE is the...

- [x] barometric pressure at a reference datum, typically the runway threshold of an airfield.
- [ ] barometric pressure adjusted to sea level, using the international standard atmosphere (ISA).
- [ ] altitude above the reference pressure level 1013.25 hPa.
- [ ] magnetic bearing to a station.

**66.** Calibrated airspeed (CAS) equals...

- [ ] true airspeed (TAS) corrected for wind.
- [x] indicated airspeed (IAS) corrected for instrument and position error.
- [ ] equivalent airspeed (EAS) corrected for altitude.
- [ ] ground speed (GS) corrected for instrument and position error.

**67.** At higher altitudes, true airspeed (TAS) tends to be higher than calibrated airspeed (CAS). A rough estimate of the TAS can be obtained by...

- [ ] subtracting 10 % of the CAS for every 1000 m altitude.
- [ ] adding 10 % of the CAS for every 1000 ft altitude.
- [ ] subtracting 2 % of the CAS for every 1000 ft altitude.
- [x] adding 2 % of the CAS for every 1000 ft altitude.

**68.** An aircraft cruises on a heading of 180° with a true airspeed of 110 kt. The wind comes from 180° with 30 kt, pressure altitude is 5000 ft. Neglecting instrument and position errors, which will be the approximate reading of the airspeed indicator?

- [ ] 110 kt
- [x] 100 kt
- [ ] 120 kt
- [ ] 90 kt

**69.** What is necessary for the determination of speed (IAS) by the airspeed indicator?

- [ ] The difference between the standard pressure and the total pressure
- [ ] The difference between the dynamic pressure and the static pressure
- [x] The difference betweeen the total pressure and the static presssure
- [ ] The difference between the total pressure and the dynamic pressure

**70.** Which of the following states the working principle of an airspeed indicator?

- [ ] Total air pressure is measured by the static ports and converted into a speed indication by the airspeed indicator
- [ ] Static air pressure is measured and compared against a vacuum.
- [x] Total air pressure is measured and compared against static air pressure.
- [ ] Dynamic air pressure is measured by the Pitot tube and converted into a speed indication by the airspeed indicator

**71.** What is the meaning of the white arc on the airspeed indicator?

- [x] Speed range for extended flaps
- [ ] Speed range in bumpy air
- [ ] Speed range not to exceed
- [ ] Speed range in smooth air

**72.** The red marking at the end of the yellow arc (5) indicates which airspeed?
  
![PFP-008](https://raw.githubusercontent.com/antiprosynthesis/ppl-a/main/figures/PFP-008.jpg)

- [ ] VS1: Stall speed with flaps up
- [x] VNE: Never-exceed speed
- [ ] VFE: Maximum flap extended speed
- [ ] VNO: Maximum speed for normal operations

**73.** The Caution Area is marked on an airspeed indicator by what color?

- [ ] Red
- [x] Yellow
- [ ] Green
- [ ] White

**74.** Where does the inclination reach its lowest value?

- [ ] At the geographic poles
- [x] At the magnetic equator
- [ ] At the geographic equator
- [ ] At the magnetic poles

**75.** The term "inclination" is defined as...

- [x] angle between earth's magnetic field lines and horizontal plane.
- [ ] deviation induced by electrical fields.
- [ ] angle between magnetic and true north.
- [ ] angle between airplane's longitudinal axis and true north.

**76.** Electronic devices on board of an aeroplane have influence on the...

- [ ] artificial horizon.
- [x] direct reading compass.
- [ ] airspeed indicator.
- [ ] turn coordinator.

**77.** The compass error caused by the aircraft's magnetic field is called...

- [ ] variation.
- [x] deviation.
- [ ] declination.
- [ ] inclination.

**78.** The indication of a magnetic compass deviates from magnetic north direction due to what errors?

- [x] Deviation, turning and acceleration errors
- [ ] Variation, turning and acceleration errors
- [ ] Inclination and declination of the earth's magnetic field
- [ ] Gravity and magnetism

**79.** An aircraft in the northern hemisphere intends to turn on the shortest way from a heading of 360° to a heading of 270°. At approximately which indication of the magnetic compass should the turn be terminated?

- [ ] 300°
- [ ] 360°
- [ ] 240°
- [x] 270°

**80.** An aircraft in the northern hemisphere intends to turn on the shortest way from a heading of 030° to a heading of 180°. At approximately which indicated magnetic heading should the turn be terminated?

- [ ] 180°
- [ ] 150°
- [x] 210°
- [ ] 360°

**81.** What behaviour is shown by a rotating gyro in space?

- [ ] It moves in accordance with the body surrounding it
- [ ] It swings from east to west like a pendulum
- [x] It tends to maintain its position in space
- [ ] It moves in circles with a stadily decreasing radius

**82.** A turn and bank (or turn and slip) coordinator provides information regarding...

- [ ] the rate of turn and bank angle of the aircraft.
- [x] the rate of turn and coordination, i.e. slip or skid, of the turn.
- [ ] the coordination of the turn and slip angle.
- [ ] the pitch and bank angle of the aircraft.

**83.** What does the slip indicator show?

- [ ] Vertical to horizon
- [x] Apparent vertical
- [ ] Airplane's bank
- [ ] Rate of turn

**84.** The bank angle of a 2-minutes circle depends on the...

- [ ] IAS.
- [x] TAS.
- [ ] Ground speed.
- [ ] CAS.

**85.** The slip indicator's ball (apparent vertical) has moved to the right during a right turn. By what actions may the ball led back to middle position?

- [x] Reduce bank, increase rate of turn
- [ ] Reduce speed, increase bank
- [ ] Increase bank, increase rate of turn
- [ ] Reduce rate of turn, reduce bank

**86.** A horizontal situation indicator (HSI) combines the information provided by...

- [ ] the attitude indicator and the flight director.
- [x] the directional gyro and the VHF navigation receiver.
- [ ] the rate gyro and the slip indicator.
- [ ] the directional gyro and the flight director.

**87.** Which light colour advises the pilot to the condition "corrective action may be required in the future"?

- [ ] Red
- [ ] Blue
- [x] Amber (orange)
- [ ] Green

**88.** What values are usually marked with a red line on instrument displays?

- [ ] Recommended areas
- [ ] Caution areas
- [x] Operational limits
- [ ] Operational areas

**89.** Which wing configuration is shown in the attachment?
  
![AGK-002](https://raw.githubusercontent.com/antiprosynthesis/ppl-a/main/figures/AGK-002.jpg)

- [x] Strut-braced high wing
- [ ] High wing
- [ ] Mid wing
- [ ] Low wing

**90.** Which tail assembly is shown in the attachment?
  
![AGK-003](https://raw.githubusercontent.com/antiprosynthesis/ppl-a/main/figures/AGK-003.jpg)

- [ ] V-tail
- [x] T-tail
- [ ] Fuselage-mounted tail
- [ ] Cruciform tail

**91.** What needs to be considered if permissible limitations were exceeded?

- [ ] The helicopter must be inspected by the pilot in command and if no defect is found, no appropriate entry in the aircraft technical log is necessary
- [ ] The helicopter must be inspected by at least two pilots licensed on the type, one of whom must be the pilot in command
- [x] The helicopter must be inspected by a qualified engineer before the next flight
- [ ] The helicopter must be subjected to a duplicate inspected by two engineers

**92.** What kind of hydraulic oil is used in aeroplane systems today?

- [ ] Mineral oil
- [ ] Vegetable oil
- [ ] Bio-oil
- [x] Synthetic oil

**93.** Where is the brake system installed to slow the aircraft on ground?

- [ ] On the tail wheel
- [ ] On the nose and main gear
- [ ] Only on the nose gear
- [x] Only on the main gear

**94.** What is checked by the marking in the attachment?
  
![AGK-006](https://raw.githubusercontent.com/antiprosynthesis/ppl-a/main/figures/AGK-006.jpg)

- [ ] The tread wear of the tire
- [ ] The operating pressure of the carcass
- [x] The correct postion of the tire relative to the rim
- [ ] The correct position of the outer isolation layers

**95.** How are the flight controls on a small single-engine piston aircraft normally controlled and actuated?

- [ ] Power-assisted through hydraulic pumps or electric motors
- [ ] Hydraulically through hydraulic pumps and actuators
- [x] Manually through rods and control cables
- [ ] Electrically through fly-by-wire

**96.** What happens to a helicopter during cruise when the stick is moved forward without other corrections?

- [x] The speed increases and the sink rate increases
- [ ] The speed decreases and the sink rate increases
- [ ] The speed increases and the sink rate decreases
- [ ] The speed decreases and the sink rate decreases

**97.** Which of the following options states all primary flight controls of an aircraft?

- [ ] All movable parts on the aircraft which aid in controlling the aircraft
- [ ] Flaps, slats, speedbrakes
- [x] Elevator, rudder, aileron
- [ ] Elevator, rudder, aileron, trim tabs, high-lift wing devices, power controls

**98.** What is the purpose of a ground adjustable trim tab?

- [ ] It is set on the ground to correct an out-of-trim condition arising from the mass and balance situation of an aircraft
- [ ] It is preset on the ground and further adjusted in flight to reduce the need to constantly re-trim the aircraft
- [ ] It is used to optimize the handling characteristics of an aircraft during ground operations
- [x] It is a non-movable metal tab on a flight control which is adjusted on the ground to optimize the inflight characteristics of an aircraft

**99.** How does a balance tab move in relation to the flight control surface that it is coupled with?

- [x] In the opposite direction
- [ ] At an angle of 90°
- [ ] At an angle of 45°
- [ ] In the same direction

**100.** The trim is used to...

- [ ] increase adverse yaw.
- [ ] move the centre of gravity.
- [x] adapt the control force.
- [ ] lock control elements.

**101.** Which of the following are an aircraft's secondary flight controls?

- [ ] Elevator, rudder, aileron
- [ ] All movable parts on the aircraft which aid in controlling the aircraft
- [ ] Elevator, rudder, aileron, trim tabs, high-lift wing devices, power controls
- [x] Wing flaps, leading edge devices, spoilers or speedbrakes, trim systems

**102.** The primer is...

- [x] an auxiliary pump in the fuel system to facilitate engine starting.
- [ ] a mechanical switch in the cockpit to engage the turbocharger.
- [ ] a valve in the fuel control system for automatic mixture regulation.
- [ ] a nozzle in the Venturi tube of a carburettor for atomising the fuel.

**103.** What must be considered if the alternator fails in a helicopter?

- [ ] All instruments and warning systems will fail
- [x] No change as long as the battery provides enough power
- [ ] The engine runs roughly and is prone to knocking
- [ ] Only high power consumers will failing

**104.** When using direct current, electrically driven flight instruments are marked with...

- [ ] "EL".
- [x] "DC".
- [ ] "AL".
- [ ] "CO".

**105.** Which cylinder arragement is commonly used on small aircrafts and motor gliders?
  
![AGK-Q105](https://raw.githubusercontent.com/antiprosynthesis/ppl-a/main/figures/AGK-Q105.jpg)

- [x] Horizontally opposed engine
- [ ] Radial engine
- [ ] In-line engine
- [ ] V-type engine

**106.** The highest absorbtion of humidity in fuel can be observed in which situation?

- [ ] During parking on wet gras areas
- [ ] During parking on cold aprons
- [x] Almost empty tanks
- [ ] Almost full tanks

**107.** What does the octane rating or fuel grade describe?

- [x] Anti-knock rating
- [ ] Flame front speed
- [ ] Ignition timing
- [ ] Combustion temperature

**108.** What is the major task of a carburettor?

- [ ] To pump fuel from the tanks into the cylinder
- [ ] To control the aircraft's speed through the throttle valve
- [ ] To provide additional fuel to cool the engine
- [x] To produce an ignitable air/fuel mixture

**109.** In which outside air temperature is icing most likely?

- [x] Between -5° C and +20° C
- [ ] Between -10° C and +10° C
- [ ] Between -20° C and +5° C
- [ ] Between -15° C and 0° C

**110.** Cylinder head temperature indication relates to...

- [ ] all Cylinders.
- [ ] a random cylinder.
- [x] the critical cylinder.
- [ ] the average of all cylinders.

**111.** Piston engines of helicopters have...

- [ ] one magneto ignition system.
- [ ] one electrical ignition system.
- [ ] two coupled ignition systems.
- [x] two independent ignition systems.

**112.** The angle indicated by arrow number 1 shows the propeller's... D: Direction of air flow C: Chord line R: Direction of rotation
  
![AGK-011](https://raw.githubusercontent.com/antiprosynthesis/ppl-a/main/figures/AGK-011.jpg)

- [x] angle of incidence.
- [ ] angle of attack.
- [ ] geometric wing twist.
- [ ] aerodynamic wing twist.

**113.** How should a power increase be executed on a constant-speed propeller, provided that no other procedure is described in the flight manual?

- [ ] 1) Decrease manifold pressure 2) Increase RPM
- [ ] 1) Decrease RPM 2) Increase manifold pressure
- [ ] 1) Increase manifold pressure 2) Increase RPM
- [x] 1) Increase RPM 2) Increase manifold pressure

**114.** How should a power decrease be executed on a constant-speed propeller, provided that no other procedure is described in the flight manual?

- [ ] 1) Decrease RPM 2) Decrease manifold pressure
- [ ] 1) Decrease manifold pressure 2) Increase RPM
- [ ] 1) Decrease RPM 2) Increase manifold pressure
- [x] 1) Decrease manifold pressure 2) Decrease RPM

**115.** With increasing altitude, the power of a carburettor engine...

- [ ] remains constant.
- [ ] first decreases, from 5000 ft on increases.
- [ ] increases.
- [x] decreases.

**116.** During ignition check the ignition is switched to OFF for a short moment and subsequently back to BOTH. What happens to the RPM if the ignition system is properly grounded?

- [ ] The RPM increases once switched to OFF and resumes a value below the previous one when switched back to BOTH
- [x] The RPM decreases once switched to OFF and resumes the previous value when switched back to BOTH
- [ ] The RPM increases once switched to OFF and resumes the previous value when switched back to BOTH
- [ ] The RPM decreases once switched to OFF and resumes a value below the previous one when switched back to BOTH

**117.** Which of the instruments listed below obtain their readings through pressure measurement?

- [x] Oil pressure gauge, fuel pressure gauge, manifold pressure gauge, altimeter, vertical speed indicator, airspeed indicator, suction gauge
- [ ] Oil pressure gauge, fuel pressure gauge, fuel quantity gauge, manifold pressure gauge, differential pressure gauge, altimeter
- [ ] Airspeed indicator, vertical speed indicator, altimeter, directional gyro, turn and bank coordinator, oil pressure gauge, fuel pressure gauge
- [ ] Airspeed indicator, vertical speed indicator, altimeter, magnetic compass, oil pressure gauge, fuel pressure gauge

**118.** A flight level is a...

- [x] pressure altitude.
- [ ] density altitude.
- [ ] altitude above ground.
- [ ] true altitude.

**119.** During a flight in colder-than-ISA air the indicated altitude is...

- [x] higher than the true altitude.
- [ ] lower than the true altitude.
- [ ] eqal to the true altitude.
- [ ] equal to the standard altitude.

**120.** During a flight in an air mass with a temperature equal to ISA and the QNH set correctly, the indicated altitude is...

- [ ] lower than the true altitude.
- [x] equal to the true altitude.
- [ ] equal to the standard atmosphere.
- [ ] higher than the true altitude.

**121.** Which of the following factors could cause an erroneous airspeed indication?

- [ ] A circuit breaker was pulled
- [ ] The earth wire is still attached
- [ ] The boost pressure line is defective
- [x] The aircraft is covered with adhesive foils

**122.** What is the meaning of the red range on the airspeed indicator?

- [ ] Speed which must not be exceeded within bumpy air
- [ ] Speed which must not be exceeded with flaps extended
- [x] Speed which must not be exceeded regardless of circumstances
- [ ] Speed which must not be exceeded in turns with more than 45° bank

**123.** What is a cause for the dip error on the direct-reading compass?

- [ ] Deviation in the cockpit
- [ ] Acceleration of the airplane
- [x] Inclination of earth's magnetic field lines
- [ ] Temperature variations

**124.** A gyro which is rotating in space responds to forces being applied to it by an evasive movement...

- [ ] at an angle of 45° to the force being applied.
- [ ] in a northern direction.
- [x] at an angle of 90° to the force being applied.
- [ ] at an angle of 180° to the force being applied.

**125.** The airspeed indicator is unservicable. The airplane may only be operated...

- [x] when the airspeed indicator is fully functional again.
- [ ] when a GPS with speed indication is used during flight.
- [ ] if no maintenance organisation is around.
- [ ] if only airfield patterns are flown.

**126.** An attitude director indicator (ADI) combines the information provided by...

- [x] the attitude indicator and the flight director.
- [ ] the directional gyro and the rate gyro.
- [ ] the attitude indicator and the relative bearing indicator.
- [ ] the attitude indicator and the radio magnetic indicator.

**127.** What does the abbreviation HSI stand for?

- [x] Horizontal Situation Indicator
- [ ] Hybernating System Indication
- [ ] Horizontal Slip Indicator
- [ ] Horrifying Steep Inclination

**128.** What is the purpose of winglets?

- [ ] Increase gliging performance at high speed.
- [ ] Increase of lift and turning manoeuvering capabilities.
- [ ] To increase efficiency of aspect ratio.
- [x] Reduction of induced drag.
